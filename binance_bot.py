...
global ETHBTC_Amount
global BNBBTC_Amount
#
ENTRY_MEAN = 100
STOP_LOSS_MEAN = 80
TAKE_PROFIT_MEAN = 120
...
###
global BalanceETH
global ETHRateBuy,ETHateSell,ETHRateNow
global entry_order_ETHBTC_ask
global entry_order_ETHBTC_bid
global ORDER_SIDE_ETHBTC
global last_ETHBTC_high_mid_Bollinger_Range
global last_ETHBTC_mid_low_Bollinger_Range
global last_ETHBTC_high_low_Bollinger_Range
global last_ETHBTC_b_up
global last_ETHBTC_b_low
###
global BalanceBNB
...
###
###
def my_get_ticker(symbol):
    myticker = {'askPrice':0.0, 'bidPrice':0.0, 'lastPrice':0.0}
    cont = 0
    ret = True
    while (ret):
        try:
            ticker =Binc.get_ticker(symbol=symbol)
            myticker = ticker
            ret = False
        except Exception as e :    
            msg = "error on get tick data("+str(e.message)+")\n"
            logger.error(msg)
            cont = cont + 1
            if (cont < 5):
                time.sleep(5)
...
def GetRateA(repeat, Target_Pair) :
   ...
   for i in range(repeat) :
       # Rate
       rate = my_get_ticker(symbol=Target_Pair)
...
#
      ...
      print("balanceETH =" + str(balanceETH))
      ...
      print("balanceBNB =" +str(balanceBNB))
       ...
       logger.error(msg)
       pass
   return balanceUSDT,balanceBTC,balanceUSDC,balanceZEN,balanceXRP,balanceETH,balanceBNB
###################
...
#
   ...
   if (symbol == BTCUSDT_Pair):
      ...
      setlot = DEC_LOT
   elif (symbol == ETHBTC_Pair):
      camount = round(float(amount),ETHBTC_DECIMAL)
      setlot = DEC_LOT
...
#
   ...
   while(OK==False):
      try:
         ...
         OK = True
         #print(order)
         #logger.info("In check_order_size(" + symbol + ") is OK:" + str(order_size))
      except Exception as e:
         ...
         #logger.error("In check_order_size(" + symbol + ") Error:" + err_mes)
...
#
   ...
   elif (Pairs == ETHBTC_Pair):
      amount = round(float(amount),ETHBTC_DECIMAL)
...
#
   if (Pairs == BTCUSDT_Pair):
      ...
      amount = round(float(abs(amount)),ETHBTC_DECIMAL)
   elif (Pairs == BNBBTC_Pair):
...
#
   global BTCJPY_price
   global BalanceUSDC,BalanceZEN,BalanceXRP,BalanceETH,BalanceBNB
   global USDCRateBuy,USDCRateSell,USDCRateNow
   global ZENRateBuy,ZENRateSell,ZENRateNow
   global XRPRateBuy,XRPRateSell,XRPRateNow
   global ETHRateBuy,ETHRateSell,ETHRateNow
   global BNBRateBuy,BNBRateSell,BNBRateNow
#
...
### 各対象ペアの最小取引量MINIMUM_LOT
   ...
   min_lot_ETHBTC = getMinLotSizeBTC(ETHBTC_Pair)
...
###
   try :
       ...
       ETHRateNow,ETHRateBuy,ETHRateSell = GetRateA(1, ETHBTC_Pair)
   ...
   try :    
       # Balance
        BalanceUSDT,BalanceBTC,BalanceUSDC,BalanceZEN,BalanceXRP,BalanceETH,BalanceBNB = GetBalanceA()
   except Exception as e :
       ret =  False
   # ログ
   if (F_Buy == True) :
...
###
   ...
   profitsBit = round(float(
       ...
       + float(BalanceETH) * float(ETHRateNow)\
   ...
   if (init):
       ...
       init_btcjpy_rate = BTCJPY_price
       init = False

   Current_Profit = profitsJPY - init_values
   msg1 = "Current Holding ON BTC:" + "{0:4f}".format(profitsBit) + "(BTC)\r\n"
   msg2 = "Current Holding ON JPY:" + "{:,}".format(profitsJPY) + "(JPY)\r\n"
   msg3 = "Current Profit  ON JPY:" + "{:,}".format(Current_Profit) + "(JPY)"
   msg4 = "Current BTC/JPY  1 BTC:" + "{:,}".format(BTCJPY_price) + "(JPY)" + " Change:" + "{:.2%}".format(float(BTCJPY_price/init_btcjpy_rate))
   #print(d+" "+msg2)
...
#
   if (notify) :
        ...
        print (d+" "+msg + "\r\n" + msg1 + msg2 + msg3 + msg4)
        logger.info(msg + msg1 + msg2 + msg3+ msg4)
   return ret
####################
def Calcurate() :
   global ORDER_SIDE_BTCUSDT
...
#
   ...
   now = datetime.today()
   d = now.strftime("%Y/%m/%d %H:%M:%S")
   ticker = my_get_ticker(symbol=BTCUSDT_Pair)
...
#
   ...
   if (dws):
      if (ORDER_SIDE_BTCUSDT == SELL):
         if (float(BalanceBTC)*0.75 <  float(KEEP_BTC)):
            dws = False
      else:
         dws = False
   if (ups==False and dws==False):
      if (round(float(BalanceBTC)*0.75,4) <  float(KEEP_BTC)):
         ups = True
#
...
####################
   ...
   # 買い注文
   if(ups):
      BTCUSDT_Amount = round(float((float(BalanceUSDT) / float(BincRateBuy)) * 0.65), 4)
      ### ここで買い再度取引収計算
      lots = False
...
######
   ...
   # 売り注文
   if (dws) :
      BTCUSDT_Amount = -round(float((float(BalanceBTC) - float(KEEP_BTC))*0.60),4)
      ### ここで売り再度取引収計算
      lots = False
...
#
   ...
   now = datetime.today()
   d = now.strftime("%Y/%m/%d %H:%M:%S")
   ticker = my_get_ticker(symbol=USDCBTC_Pair)
   ask= float(ticker['askPrice']) 
   bid =float(ticker['bidPrice'])
   last=float(ticker['lastPrice'])
   if (last_USDCBTC_high_mid_Bollinger_Range == 0.0000):
      ...
      if ((b_up - b_low) > last_USDCBTC_high_low_Bollinger_Range) :
         ...
         if (last < b_mid and last < b_mid - ENTRY_MEAN*LOTOF_USDCBTC and open > last):
            if (closes[-2] > closes[-1] and (last - last_b_low) > (last - b_low) and last < (b_mid + b_low)/UpRange) :
                ups = True
         if (last > b_mid and last > b_mid + ENTRY_MEAN*LOTOF_USDCBTC and open < last):
            if (closes[-2] < closes[-1] and (last_b_up - last) > (b_up - last) and last > (b_up + b_mid)/DwRange) :
...
#
   if (ups):
       if (ORDER_SIDE_USDCBTC == SELL):
            if (round(float(ask),4) > round(float(entry_order_USDCBTC_ask + STOP_LOSS_MEAN * LOTOF_USDCBTC),4)):
                ups = False
   if (dws):
       if (ORDER_SIDE_USDCBTC == BUY):
            if (round(float(bid),4) < round(float(entry_order_USDCBTC_bid - STOP_LOSS_MEAN * LOTOF_USDCBTC),4)):
                dws = False                
...
#
   ...
   now = datetime.today()
   d = now.strftime("%Y/%m/%d %H:%M:%S")
   ticker = my_get_ticker(symbol=ZENBTC_Pair)
   ask= float(ticker['askPrice']) 
   bid =float(ticker['bidPrice'])
   last=float(ticker['lastPrice'])
   if (last_ZENBTC_high_mid_Bollinger_Range == 0.0000):
      ...
      if ((b_up - b_low) > last_ZENBTC_high_low_Bollinger_Range) :
         ...
         if (last < b_mid and last < b_mid - ENTRY_MEAN*LOTOF_ZENBTC and open > last):
            if (closes[-2] > closes[-1] and (last - last_b_low) > (last - b_low) and last < (b_mid + b_low)/UpRange) :
                ups = True
         if (last > b_mid and last > b_mid + ENTRY_MEAN*LOTOF_ZENBTC and open < last):
            if (closes[-2] < closes[-1] and (last_b_up - last) > (b_up - last) and last > (b_up + b_mid)/DwRange) :
...
#
   if (ups):
       if (ORDER_SIDE_ZENBTC == SELL):
            if (round(float(ask),3) > round(float(entry_order_ZENBTC_ask + STOP_LOSS_MEAN * LOTOF_ZENBTC),3)):
                ups = False
   if (dws):
       if (ORDER_SIDE_ZENBTC == BUY):
            if (round(float(bid),3) < round(float(entry_order_ZENBTC_bid - STOP_LOSS_MEAN * LOTOF_ZENBTC),3)):
                dws = False
#
   if (ORDER_SIDE_ZENBTC == BUY and ups == False):
       if (round(float(bid),3) > round(float(entry_order_ZENBTC_bid + TAKE_PROFIT_MEAN * LOTOF_ZENBTC),3)):
            print("ZENBTC:BUY:TAKE_PROFIT:current_bid=" + str(bid) + ":entry_bid=" + str(entry_order_ZENBTC_bid))
            #logger.info("ZENBTC:BUY:TAKE_PROFIT:current_bid=" + str(bid) + ":entry_bid=" + str(entry_order_ZENBTC_bid))
            ups = True
   elif (ORDER_SIDE_ZENBTC == SELL and dws == False):
       if (round(float(ask),3) < round(float(entry_order_ZENBTC_ask - TAKE_PROFIT_MEAN * LOTOF_ZENBTC),3)):
            print("ZENBTC:SELL:TAKE_PROFIT:current_ask=" + str(ask) + ":entry_ask=" + str(entry_order_ZENBTC_ask))
            #logger.info("ZENBTC:SELL:TAKE_PROFIT:current_ask=" + str(ask) + ":entry_ask=" + str(entry_order_ZENBTC_ask))
            dws = True
#
...
#
   ...
   now = datetime.today()
   d = now.strftime("%Y/%m/%d %H:%M:%S")
   ticker = my_get_ticker(symbol=XRPBTC_Pair)
   ask= float(ticker['askPrice']) 
   bid =float(ticker['bidPrice'])
   last=float(ticker['lastPrice'])
   if (last_XRPBTC_high_mid_Bollinger_Range == 0.0000):
      ...
      if ((b_up - b_low) > last_XRPBTC_high_low_Bollinger_Range) :
         ...
         if (last < b_mid and last < b_mid - ENTRY_MEAN*LOTOF_XRPBTC and open > last):
            if (closes[-2] > closes[-1] and (last - last_b_low) > (last - b_low) and last < (b_mid + b_low)/UpRange) :
               ups = True
         if (last > b_mid and last > b_mid + ENTRY_MEAN*LOTOF_XRPBTC and open < last):
            if (closes[-2] < closes[-1] and (last_b_up - last) > (b_up - last) and last > (b_up + b_mid)/DwRange) :
...
#
   if (ups):
       if (ORDER_SIDE_XRPBTC == SELL):
            if (round(float(ask),4) > round(float(entry_order_XRPBTC_ask + STOP_LOSS_MEAN * LOTOF_XRPBTC),4)):
                ups = False
   if (dws):
       if (ORDER_SIDE_XRPBTC == BUY):
            if (round(float(bid),4) < round(float(entry_order_XRPBTC_bid + STOP_LOSS_MEAN * LOTOF_XRPBTC),4)):
                dws = False
#
   if (ORDER_SIDE_XRPBTC == BUY and ups == False):
       if (round(float(bid),4) > round(float(entry_order_XRPBTC_bid + TAKE_PROFIT_MEAN * LOTOF_XRPBTC),4)):
            print("XRPBTC:BUY:TAKE_PROFIT:current_bid=" + str(bid) + ":entry_bid=" + str(entry_order_XRPBTC_bid))
            #logger.info("XRPBTC:BUY:TAKE_PROFIT:current_bid=" + str(bid) + ":entry_bid=" + str(entry_order_XRPBTC_bid))
            ups = True
   elif (ORDER_SIDE_XRPBTC == SELL and dws == False):
       if (round(float(ask),4) < round(float(entry_order_XRPBTC_ask - TAKE_PROFIT_MEAN * LOTOF_XRPBTC),4)):
            print("XRPBTC:SELL:TAKE_PROFIT:current_ask=" + str(ask) + ":entry_ask=" + str(entry_order_XRPBTC_ask))
            #logger.info("XRPBTC:SELL:TAKE_PROFIT:current_ask=" + str(ask) + ":entry_ask=" + str(entry_order_XRPBTC_ask))
            dws = True
#
...
####################
########## ***** SUB TARGET ETHBTC ***** ##########
   print("Calcurate ETHBTC Chart for Execute Market Order on Signal Checking ***")
#
   ETHBTC_Amount = 0.0000
   ups = False
   dws = False
   lots = False
#
   ...
   b_mid = 0.00000
   # 1分足チヤートデータ取得
   chart = get_chart_data(SYMBOL_ETHBTC)
   time.sleep(5)
   index = 4  ## INDEX(4):CLOSE値
   if (len(chart)>0):
      # get the ohlCv (closing price, index == 4)
...
#
   ...
   now = datetime.today()
   d = now.strftime("%Y/%m/%d %H:%M:%S")
   ticker = my_get_ticker(symbol=ETHBTC_Pair)
   ask= float(ticker['askPrice']) 
   bid =float(ticker['bidPrice'])
   last=float(ticker['lastPrice'])
   if (last_ETHBTC_high_mid_Bollinger_Range == 0.0000):
      last_ETHBTC_high_mid_Bollinger_Range = (b_up - b_mid)
      last_ETHBTC_mid_low_Bollinger_Range = (b_mid - b_low)
      last_ETHBTC_high_low_Bollinger_Range = (b_up - b_low)
   else:
      if ((b_up - b_low) > last_ETHBTC_high_low_Bollinger_Range) :
         if (last_ETHBTC_high_mid_Bollinger_Range < b_up - b_mid) :
            if (last < b_mid):
               ups = True
         if (last_ETHBTC_mid_low_Bollinger_Range < (b_mid - b_low)):
            if (last > b_mid):
               dws = True
      else:
         if (last < b_mid and last < b_mid - ENTRY_MEAN*LOTOF_ETHBTC and open > last):
            if (closes[-2] > closes[-1] and (last - last_b_low) > (last - b_low) and last < (b_mid + b_low)/UpRange) :
               ups = True
         if (last > b_mid and last > b_mid + ENTRY_MEAN*LOTOF_ETHBTC and open < last):
            if (closes[-2] < closes[-1] and (last_b_up - last) > (b_up - last) and last > (b_up + b_mid)/DwRange) :
      ...
      last_ETHBTC_mid_low_Bollinger_Range = (b_mid - b_low)
      last_ETHBTC_high_low_Bollinger_Range = (b_up - b_low)
      last_ETHBTC_b_up = b_up
      last_ETHBTC_b_low = b_low
#
   if (ups):
       if (ORDER_SIDE_ETHBTC == BUY):
          ups = False
   if (dws):
       if (ORDER_SIDE_ETHBTC == SELL):
          dws = False
#
   if (ups):
       if (ORDER_SIDE_ETHBTC == SELL):
            if (round(float(ask),4) > round(float(entry_order_ETHBTC_ask + STOP_LOSS_MEAN * LOTOF_ETHBTC),4)):
                ups = False
   if (dws):
       if (ORDER_SIDE_ETHBTC == BUY):
            if (round(float(bid),4) < round(float(entry_order_ETHBTC_bid + STOP_LOSS_MEAN * LOTOF_ETHBTC),4)):
                dws = False
#
   if (ORDER_SIDE_ETHBTC == BUY and ups == False):
       if (round(float(bid),4) > round(float(entry_order_ETHBTC_bid + TAKE_PROFIT_MEAN * LOTOF_ETHBTC),4)):
            print("ETHBTC:BUY:TAKE_PROFIT:current_bid=" + str(bid) + ":entry_bid=" + str(entry_order_ETHBTC_bid))
            #logger.info("ETHBTC:BUY:TAKE_PROFIT:current_bid=" + str(bid) + ":entry_bid=" + str(entry_order_ETHBTC_bid))
            ups = True
   elif (ORDER_SIDE_ETHBTC == SELL and dws == False):
       if (round(float(ask),4) < round(float(entry_order_ETHBTC_ask - TAKE_PROFIT_MEAN * LOTOF_ETHBTC),4)):
            print("ETHBTC:SELL:TAKE_PROFIT:current_ask=" + str(ask) + ":entry_ask=" + str(entry_order_ETHBTC_ask))
            #logger.info("ETHBTC:SELL:TAKE_PROFIT:current_ask=" + str(ask) + ":entry_ask=" + str(entry_order_ETHBTC_ask))
            dws = True
#
   print(ETHBTC_Pair + " Upper signal = " + str(ups))
   print(ETHBTC_Pair + " Downd signal = " + str(dws))
####################
   #print("************* Checking Order Signal *******************")
   # 買い条件を決めているところ
   # 買い注文
   if(ups):
      ETHBTC_Amount = round(float((float(BalanceBTC) - float(KEEP_BTC)) / float(ETHRateBuy) * 0.75), 4)
      #print("Buy ETHBTC_Amount=" + str(ETHBTC_Amount))
      ### ここで買い再度取引収計算
      lots = False
      if(ETHBTC_Amount >= min_lot_ETHBTC):
         lots = True
         ret = check_order_size(BUY, ETHBTC_Pair, ETHBTC_Amount, ask)
         if (ret['OK']):
            ETHBTC_Amount = float(ret['OrderSize'])
         else:
            lots = False
      if(lots):
         if (ETHBTC_Amount > MAX_ALT) :
            ETHBTC_Amount = MAX_ALT
         msg = "Calcurate ETHBTC_Amount(Buy):"+str(ETHBTC_Amount)
         print (d+" "+msg)
      else:
         ETHBTC_Amount = 0.0000
########
   ...
   # 売り注文
   if (dws) :
      ETHBTC_Amount = -round(float(BalanceETH) - 0.005, 4)
      ### ここで売り再度取引収計算
      lots = False
      if(abs(ETHBTC_Amount) >= min_lot_ETHBTC):
         lots = True
      ret = check_order_size(SELL, ETHBTC_Pair, abs(ETHBTC_Amount), bid)
      if (ret['OK']):
         ETHBTC_Amount = -float(ret['OrderSize'])
      else:
         lots = False
   if(lots) :
       if (abs(ETHBTC_Amount) > MAX_ALT) : # 最低限必要な量（要確認）
           ETHBTC_Amount = - MAX_ALT
       msg = "Calcurate ETHBTC_Amount(Sell):"+str(ETHBTC_Amount)
       print (d+" "+msg)
   else:
       ETHBTC_Amount = 0.0000
#
#
   print("ETHBTC_Amount=" + str(ETHBTC_Amount))
###################
###################
########## ***** SUB TARGET BNBBTC ***** ##########
...
####################
   ...
   if (float(BalanceBNB) < MIN_BNB):
       ...
       if (ret['OK']):
            ...
            BNBBTC_Amount = round(float((float(BalanceBTC) - float(KEEP_BTC) / float(BNBRateBuy)) * 0.9), 4)
            #print("Buy BNBBTC_Amount=" + str(BNBBTC_Amount))
            lots = False
            if(BNBBTC_Amount >= min_lot_BNBBTC and BNBBTC_Amount < MIN_BNB):
                lots = True
                ret = check_order_size(BUY, BNBBTC_Pair, abs(BNBBTC_Amount), ask)
                if (ret['OK']):
                    BNBBTC_Amount = float(ret['OrderSize'])
...
#
       ...
       order_mark = 0
#####
       if(ORDER_SIDE_BTCUSDT == BUY and entry_order_BTCUSDT_ask > 0.0 and long_side_position > 0.0):
...
#
           if(order_mark>0 and long_side_position > MIN_BTC):
                OrderOK = False
                BTCUSDT_Amount = long_side_position
                ret = check_order_size(SELL, BTCUSDT_Pair, BTCUSDT_Amount, current_bid)
                if (ret['OK']):
                    OrderOK = True
                    BTCUSDT_Amount = float(ret['OrderSize'])
                else:
                    BTCUSDT_Amount = round(float((float(BalanceBTC) - float(KEEP_BTC))*0.75),4)
                    ret = check_order_size(SELL, BTCUSDT_Pair, BTCUSDT_Amount, current_bid)
                    if (ret['OK']):
                        OrderOK = True
                        BTCUSDT_Amount = float(ret['OrderSize'])
...
#
######
       if(ORDER_SIDE_BTCUSDT == SELL and entry_order_BTCUSDT_bid > 0.0 and short_side_position > 0.0):
...
#
           if(order_mark>0 and short_side_position > MIN_BTC):
                OrderOK = False
                BTCUSDT_Amount = short_side_position
                ret = check_order_size(BUY, BTCUSDT_Pair, BTCUSDT_Amount, current_ask)
                if (ret['OK']):
                    OrderOK = True
                    BTCUSDT_Amount = float(ret['OrderSize'])
                else:
                    BTCUSDT_Amount = round(float((float(BalanceUSDT) / float(BincRateBuy)) * 0.70), 4)
                    ret = check_order_size(SELL, BTCUSDT_Pair, BTCUSDT_Amount, current_ask)
                    if (ret['OK']):
                        OrderOK = True
                        BTCUSDT_Amount = float(ret['OrderSize'])
                if (OrderOK):
...
###
       ...
       try : 
           ...
           pass
#######################################################
########## SUb Target ETHBTC Order Logic ############
#
   if (ETHBTC_Amount > 0.0000) :
       try :
           # 買い注文
           BuyPrice = ETHRateBuy
           SellPrice = ETHRateSell
           ### MarketでBuy注文 
           ret = BuyMarket(ETHBTC_Amount, SYMBOL_ETHBTC, ETHBTC_Pair, BuyPrice)
           if (ret == False):
               ret = check_order_size(BUY, ETHBTC_Pair, ETHBTC_Amount, BuyPrice)
               if (ret['OK']):
                  if (ret['OrderSize'] != ETHBTC_Amount):
                     ETHBTC_Amount = float(ret['OrderSize'])
                  ret = BuyMarket(ETHBTC_Amount, SYMBOL_ETHBTC, ETHBTC_Pair, BuyPrice)
               else:
                  ret = False
           if(ret):
               ORDER_SIDE_ETHBTC = BUY
               ticker = my_get_ticker(symbol=ETHBTC_Pair)
               entry_order_ETHBTC_ask= float(ticker['askPrice']) 
               entry_order_ETHBTC_bid =float(ticker['bidPrice'])
               BalanceBought = BalanceBought + float((float(ETHBTC_Amount) * float(ETHRateNow)) * 0.1)
               time.sleep(20)
       except Exception as e :
           msg = "注文（買い）に失敗しました。@" + SYMBOL_ETHBTC + "@" +str(e.message)
           print (d+" "+msg)
           #logger.error(msg)
           pass
###
   elif (ETHBTC_Amount < 0.0000) :
       try : 
           # 売り注文
           BuyPrice = ETHRateBuy
           SellPrice = ETHRateSell
           ### MarketでSell注文 
           ret = SellMarket(ETHBTC_Amount, SYMBOL_ETHBTC, ETHBTC_Pair, SellPrice)
           if (ret == False):
               ret = check_order_size(SELL, ETHBTC_Pair, abs(ETHBTC_Amount), SellPrice)
               if (ret['OK']):
                  if (ret['OrderSize'] != abs(ETHBTC_Amount)):
                     ETHBTC_Amount = - float(ret['OrderSize'])
                  ret = SellMarket(ETHBTC_Amount, SYMBOL_ETHBTC, ETHBTC_Pair, SellPrice)
               else:
                  ret = False
           if(ret):
               ORDER_SIDE_ETHBTC = SELL
               ticker = my_get_ticker(symbol=ETHBTC_Pair)
               entry_order_ETHBTC_ask= float(ticker['askPrice']) 
               entry_order_ETHBTC_bid =float(ticker['bidPrice'])
               BalanceBought = BalanceBought + float((float(ETHBTC_Amount) * float(ETHRateNow)) * 0.05)
               time.sleep(20)
       except Exception as e :
           msg = "注文（売り）に失敗しました。@" + SYMBOL_ETHBTC + "@" +str(e.message)
           print (d+" "+msg)
           #logger.error(msg)
           pass
#######################################################
   ...
   if (BNBBTC_Amount > 0.0000) :
       try :
           ...
           if(ret):
               ...
               ticker = my_get_ticker(symbol=BNBBTC_Pair)
...
#
   ...
   last_ETHBTC_b_up = 0.0
   last_ETHBTC_b_low = 0.0
   ...
   last_ETHBTC_high_mid_Bollinger_Range = 0.0000
   last_ETHBTC_mid_low_Bollinger_Range = 0.0000
   last_ETHBTC_high_low_Bollinger_Range = 0.0000
...
#
   ...
   entry_order_ETHBTC_ask = 0.0
   entry_order_ETHBTC_bid = 0.0
